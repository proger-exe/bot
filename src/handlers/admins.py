...
async def ban_moder(msg: types.Message):
    try:
        ...
        if user.is_chat_admin():
            return await msg.reply("Забанить админа? Хорошая идея, но пожалуй, вы справитесь с этим сами.")
...
async def mute_moder(msg: types.Message):
    try:
        ...
        if user.is_chat_admin():
            return  await msg.answer("Замутить админа? Хорошая идея, но пожалуй, вы справитесь с этим сами..")
